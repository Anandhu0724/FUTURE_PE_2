...
app.post("/api/generate-voice", async (req, res) => {
  try {
    ...
    if (!text) {
      return res.status(400).json({ error: "Missing required speech text content" });
    }
    ...
    if (!base64Audio) {
      return res.status(500).json({ error: "Failed to synthesize speech. No audio data was generated by the model." });
    }
    ...
    res.status(500).json({ error: err.message || "An error occurred during AI voiceover synthesis." });
  }
});
